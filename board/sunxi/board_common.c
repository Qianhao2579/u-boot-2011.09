...
int set_power_off_use_gpio(void);
/*
************************************************************************************************************
*
*                                             function
...
*    return        :
*
*    note          : v3s plafrom decides go the fel mode with normal gpio
*
*
************************************************************************************************************
*/
int check_update_key_use_gpio(void)
{
    user_gpio_set_t	fel_gpio ;
    __u32 gpio_hd;
    int ret;
    int gpio_value = 0;
    int mode = 0;
    ...
    ret = script_parser_fetch("fel_key","fel_key_used",&used, sizeof(int) / 4);
    if(ret || used != 1)
    {
        return 0;
    }
    fel_gpio.port = NULL;
    ret = script_parser_fetch("fel_key","fel_gpio", (int *)&fel_gpio, sizeof(user_gpio_set_t) / 4);
    if(!ret && fel_gpio.port != NULL)
    {
        fel_gpio.mul_sel = 0;		//强制设置成输入
        gpio_hd = gpio_request(&fel_gpio, 1);
        if (gpio_hd)
        {
            int time;
            gpio_value = 0;
            ...
            if (!gpio_value)
            {
                printf("find fel key and go to fel mode right now\n");
                return sunxi_board_run_fel();
            }
        }
    }
    return 0;
...
*    return        :
*
*    note          : v3s plafrom shutdown power by gpio without axp
*
*
************************************************************************************************************
*/
int set_power_off_use_gpio(void)
{
	...
	ret = script_parser_fetch("power_gpio", "used", &used, sizeof(int) / 4);
    if (ret || (used == 0))
    {
        return 0;
    }
    gpio_hd = gpio_request_ex("power_gpio", NULL);
    if (gpio_hd)
    {
        tick_printf("gpio power off\n");
        gpio_write_one_pin_value(gpio_hd, power_on_gpio.data==0?1:0,"power_on");
        return 0;
    }
    return -1;
}
...
int check_update_key(void)
{
    ...
    //if config normal gpio to decide whether enter fel-mode,skip button set of power-key and adc-key detection
    //normal gpio fel key detection will call at board_init_r
    fel_gpio.port = NULL;
    ret = script_parser_fetch("fel_key","fel_gpio", (int *)&fel_gpio, sizeof(user_gpio_set_t) / 4);
    if(!ret && fel_gpio.port != NULL )
    {
