...
{
#ifdef __LINUX__OS__
	volatile unsigned int *tcon_reg = (unsigned int *)(0xf1c0c000);
#else
	volatile unsigned int *tcon_reg = (unsigned int *)(0x01c0c000);
#endif
	int size = 0x180>>2;
	int val, i, sel;
	char output[128];
	...
	while (size>0) {
		...
		p = output;
		p += sprintf(p, "0x%08x:", (unsigned int)tcon_reg);
		for (i=0; i<sel; i++) {
			val = *(tcon_reg);
			p += sprintf(p, " %08x", val);
...
static void lcd_panel_ili9341_init(disp_panel_para *info)
{
	...
	printf("[ili9341]:###this is for test ili9341 \n");
	sunxi_lcd_gpio_set_value(0, 1, 0);

	if (!bootup_flag) {
		bootup_flag = 1;
		printf("ili9341 soft reset\n");
	...
	printf("ili9341 read signal mode:\n");
	printf("ili9341 0x%02x 0x%02x\n", id[1], id[0]);
#endif
