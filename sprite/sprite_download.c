...
int sunxi_sprite_download_boot0_simple(void)
{
	...
	#endif
    if(production_media == 5)
        spinand_upload_boot0(200 * 1024, buffer);
	else
	...
	if(!gd->securemode)
    {
        ...
        if(production_media == 5)
        {
			return spinand_download_boot0_simple(boot0->boot_head.length, buffer);
        }
		else
		{
			return card_download_boot0(boot0->boot_head.length, buffer);
	...
	else
	{
		...
		if(production_media == 5)
		{
			return spinand_download_boot0_simple(toc0->length, buffer);
		}
		else
		{
			return card_download_boot0(toc0->length, buffer);
