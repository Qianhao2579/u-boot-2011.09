...
ifdef CONFIG_STORAGE_MEDIA_SPINAND
		@git show HEAD --pretty=format:"%H" | head -n 1 > cur.log
		@./add_hash.sh -f sunxi_spl/boot0/boot0_spinand.bin -m boot0
		@$(TOPDIR)/tools/gen_check_sum sunxi_spl/boot0/boot0_spinand.bin boot0_spinand_$(CONFIG_TARGET_NAME).bin > /dev/null
		@if [ -z "$(findstring $(OTA_TEST_NAME), $(SUNXI_MODE))" ]; then \
			cp -v boot0_spinand_$(CONFIG_TARGET_NAME).bin $(TOPDIR)/../tools/pack/chips/$(CONFIG_TARGET_NAME)/bin/boot0_spinand_$(CONFIG_TARGET_NAME).bin; \
		else \
			cp -v boot0_spinand_$(CONFIG_TARGET_NAME).bin $(TOPDIR)/../tools/pack/chips/$(CONFIG_TARGET_NAME)/bin/boot0_spinand_$(CONFIG_TARGET_NAME)-$(SUNXI_MODE).bin; \
		fi
endif
