...
int power_gpio_enable(void)
{
       user_gpio_set_t gpio_power_on;
       int used = 0, ret = -1;
       __u32 hd_power_on = 0;
       ...
       printf("try power_gpio_enable\n");
       memset(&gpio_power_on, 0, sizeof(user_gpio_set_t));
       ret = script_parser_fetch("power_gpio", "power_on", (void *)&gpio_power_on, sizeof(user_gpio_set_t)>>2);
       if(!ret)
       {
               if(gpio_power_on.port)
               {
                       hd_power_on = gpio_request(&gpio_power_on, 1);
                       if(!hd_power_on)
                       {
                               printf("reuqest gpio_power_on failed\n");
                               return -1;
                       }
                       gpio_write_one_pin_value(hd_power_on, 1, "power_on");
                       printf("enable power_on gpio success!");
